...
if (isset($_GET['ajax']) && $_GET['ajax'] == 'get_attendance') {
    ...
    while ($currentWeek <= 6) {
        ...
        $stmt = $conn->prepare($query);
        $stmt->bind_param('ssss', $lrn, $subject, $adjustedStartDate, $adjustedEndDate);
        $stmt->execute();
        $result = $stmt->get_result();
...
<script>
    ...
    let attendanceData = []; // Store monthly attendance data for export
    ...
    function exportToCSV() {
        ...
        attendanceData.forEach(month => {
            if (month.label !== 'No attendance records found') {
                // Add monthly data
                csvContent += `"${month.label}",${month.present},${month.absent}\r\n`;
                ...
                if (monthRow && monthRow.classList.contains('expanded') && weeklyData[month.label]) {
                    ...
                    weeklyData[month.label].forEach(week => {
                        csvContent += `"  ${week.label}",${week.present},${week.absent}\r\n`;
                    });
                }
            }
        });
        ...
        link.setAttribute("download", `attendance_${subject.replace(/\s+/g, '_')}_${studentLRN}.csv`);
